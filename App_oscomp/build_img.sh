...
echo "Creating disk image of size $size MB, filesystem: $fs, architecture: $arch, files: $FILE"
dd if=/dev/zero of=disk.img bs=4M count=$size

if [ "$fs" = "ext4" ]; then
  # TODO: support metadata_csum
	mkfs.ext4 -O ^metadata_csum -t ext4 disk.img
